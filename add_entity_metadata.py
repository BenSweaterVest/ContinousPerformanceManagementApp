#!/usr/bin/env python3
"""
Add required entity metadata to all custom entities
Based on Microsoft Teams Dataverse sample format
"""
...
# Entity metadata template (based on Microsoft samples)
ENTITY_METADATA = """          <EntitySetName>{entity_set_name}</EntitySetName>
...
# Entity names mapping (PascalCase -> entity_set_name)
ENTITIES = {
    "pm_StaffMember": "pm_staffmembers",
    "pm_EvaluationQuestion": "pm_evaluationquestions",
    "pm_WeeklyEvaluation": "pm_weeklyevaluations",
    "pm_SelfEvaluation": "pm_selfevaluations",
    "pm_IDPEntry": "pm_idpentrys",
    "pm_MeetingNote": "pm_meetingnotes",
    "pm_Goal": "pm_goals",
    "pm_Recognition": "pm_recognitions",
    "pm_ActionItem": "pm_actionitems",
}

print("Adding entity metadata to all custom entities...")
print()

# Find and replace each entity
for pascal_name, entity_set_name in ENTITIES.items():
    # Create metadata for this entity
    metadata = ENTITY_METADATA.format(entity_set_name=entity_set_name)

    # Find the pattern: </attributes>\n        </entity>
    # Replace with: </attributes>\n{metadata}\n        </entity>
    pattern = f'(<entity Name="{pascal_name}">.*?</attributes>)\n(\\s+</entity>)'
    replacement = f'\\1\n{metadata}\\2'
    ...
    if new_len > old_len:
        print(f"✓ Added metadata to {pascal_name}")
    else:
        print(f"✗ Failed to add metadata to {pascal_name}")
...
print("✓ Entity metadata added to all 9 entities")
